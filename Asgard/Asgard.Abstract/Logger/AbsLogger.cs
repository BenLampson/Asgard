...
namespace Asgard.Abstract.Logger
{
    /// <summary>
    /// 容器日志抽象
    /// </summary> 
    public abstract class AbsLogger : IDisposable
    {
        ...
        /// 自定义日志格式化函数
        /// </summary>
        public Func<LogLevelEnum, string, string?, Exception?, string, int, string, string>? CustomLogFormatter { get; set; }
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        /// <param name="name">函数名</param>
        /// <param name="num">行数</param>
        protected virtual string GetLogFullString(LogLevelEnum level, string text, string? eventID, Exception? exception, string filePath, int num, string name)
        {
            if (CustomLogFormatter is not null)
            {
                return CustomLogFormatter(level, text, eventID, exception, filePath, num, name);
            }
    ...
    public abstract class AbsLogger<T> : AbsLogger where T : class
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        /// <param name="moduleName">模块名称</param>
        protected AbsLogger(string moduleName) : base(moduleName) { }
    }
}
