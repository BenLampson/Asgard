...
using Asgard.Core.SystemManagerModules.Partial;
...
namespace Asgard.Core.SystemManagerModules
{
    ...
    public partial class AsgardSystemManager
    {
        ...
        /// 初始化DI数据
        /// </summary> 
        private void InitDI(WebApplicationBuilder builder, LoggerProvider provider, DataBaseManager db, Func<AsgardContext> contextCreator)
        {
            _ = builder.Services.AddSingleton(_ => provider);
            _ = builder.Services.AddSingleton(_ => db);
            _ = builder.Services.AddScoped<AsgardContext>(_ => contextCreator());
