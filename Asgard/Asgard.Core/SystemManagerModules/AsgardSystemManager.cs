...
namespace Asgard.Core.SystemManagerModules
{
    /// <summary>
    /// 一个阿斯加德系统的管理器,你可以利用该系统快速启动项目
    /// 注意,为了保持系统的一致性,就算是配置中心自己,本质也是自己启动了自己
    /// </summary>
    public partial class AsgardSystemManager
    {
        /// <summary>
        /// 当前启动事件ID
        /// </summary>
        ...
        public AsgardSystemManager(string[] systemArgs)
        {
        ...
        /// </summary>
        public AsgardSystemManager Start()
        {
        ...
        public async void StartHost()
        {
            ...
            if (WebApp is not null)//web宿主
            {
                _ = WebApp.Lifetime.ApplicationStarted.Register(() =>
                {
                    ...
                    {
                        LoggerProvider?.CreateLogger<AsgardSystemManager>().Critical("系统启动失败!", exception: ex, eventID: EventID);
                ...
                _ = WebApp.Lifetime.ApplicationStopping.Register(() =>
                {
                    ...
                    {
                        LoggerProvider?.CreateLogger<AsgardSystemManager>().Critical("系统停止失败!", exception: ex, eventID: EventID);
                    }
                });
                LoggerProvider?.CreateLogger<AsgardSystemManager>().Critical("系统启动.", eventID: EventID);
                WebApp.Start();
            }
            ...
            LoggerProvider?.CreateLogger<AsgardSystemManager>().Critical("系统已停止.", eventID: EventID);
